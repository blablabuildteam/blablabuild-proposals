...
export function SlidePhaseBacklog() {
  const { phases, getWorkflow, slideCopy } = useProposal();
  const ui = useProposalUi();
  const phase = phases.find((p) => p.id === "backlog");
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="rounded-xl border border-dashed border-[var(--brand-border)] bg-[var(--brand-bg)] px-5 py-3 text-right">
          <p className="font-mono text-xl font-bold text-[var(--brand-fg)] sm:text-2xl">
            {phase.invest}
          </p>
          <p className="text-xs font-medium text-[var(--brand-muted)]">{phase.period}</p>
        </div>
      </div>

      <div className="mb-5 rounded-xl border border-dashed border-[var(--brand-border)] bg-white p-4 sm:p-5">
      ...
      <p className="mt-4 text-xs text-[var(--brand-muted)]">
        {slideCopy?.phaseBacklog?.note ??
          "Backlog items follow once the foundation from earlier phases is in place."}
      </p>
    </div>
  );
}
